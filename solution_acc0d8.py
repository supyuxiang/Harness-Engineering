...
class MyHarness(Harness):
    def __init__(self, call_llm, count_tokens, count_messages_tokens, max_prompt_tokens: int,tokenize_fn_name:str='default',sim_fn_name:str='jaccard'):
    ...
    def retrieve(self, text: str, top_k: int = 20):
        ...
        if not scored:
            m = min(top_k, len(self.example_cache))
        ...
        for _, _, example in scored:
            ...
            if len(selected) >= top_k:
                break
        return selected if selected else [x[2] for x in scored[:top_k]]
    ...
    def predict(self, text: str) -> str:
        ...
        retrieved_examples = self.retrieve(text, top_k=20)
