...
def count_tokens(text: str) -> int:
    """计算单段文本的 token 数。"""
    global _tokenizer, _tokenizer_loaded, _tokenizer_unavailable
    if not text:
        return 0
    ...
    if not _tokenizer_loaded:
        with _tokenizer_lock:
            if not _tokenizer_loaded and not _tokenizer_unavailable:
                try:
                    ...
                    return max(1, len(text) // 4)
    return len(_tokenizer(text, add_special_tokens=False)["input_ids"]) if _tokenizer is not None else max(1, len(text) // 4)
